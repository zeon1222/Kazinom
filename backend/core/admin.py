...
# 3) /confirm  (withdrawal və ya deposit üçün ortaq)
...
def confirm_request(req: RequestAction):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(f"SELECT * FROM {table} WHERE id = %s", (req.request_id,))
        row = cur.fetchone()
        if not row:
        ...
        cur.execute(
            f"UPDATE {table} SET status = 'completed', completed_at = now() WHERE id = %s",
            (req.request_id,),
        )
...
# 4) /reject  (withdrawal-da balans geri qayıdır, deposit-də sadəcə rədd olunur)
@router.post("/reject")
def reject_request(req: RequestAction):
    ...
    with get_db() as conn:
        ...
        if row["status"] != "pending":
            raise HTTPException(status_code=400, detail=f"Tələb artıq '{row['status']}' statusundadır.")
        cur.execute(
            f"UPDATE {table} SET status = 'rejected', completed_at = now() WHERE id = %s",
            (req.request_id,),
        )
    ...
    if req.kind == "withdrawal":
        ...
        send_telegram_message(row["telegram_id"], f"❌ Depozit tələbiniz rədd edildi.")
