...
# RTP hər üçü də ~95% civarında, əvvəlkindən bir az aşağı (server xeyrinə)
RISK_TABLES = {
    "LOW":    [2.5, 1.4, 1.1, 0.9, 0.7, 0.7, 0.9, 1.1, 1.4, 2.5],    # RTP ~95%
    "MEDIUM": [10,  3.0, 1.6, 0.9, 0.4, 0.4, 0.9, 1.6, 3.0, 10],     # RTP ~95%
    "HIGH":   [55,  8,   1.4, 0.25, 0.1, 0.1, 0.25, 1.4, 8, 55],     # RTP ~95%
}
DEFAULT_RISK = "MEDIUM"
MIN_BET_QEPIK = 5
MAX_BET_QEPIK = 10000
...
def place_bet(telegram_id: str, bet_qepik: int, risk: str):
    ...
    with get_db() as conn:
        cur = conn.cursor()
        # nonce-u atomik artır ki paralel sorğularda təkrar istifadə olunmasın
        cur.execute(
        ...
        net_delta = payout_qepik - bet_qepik
